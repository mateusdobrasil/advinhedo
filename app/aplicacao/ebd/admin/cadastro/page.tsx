...
export default async function CadastroCentralPage({ searchParams }: PageProps) {
  ...
  const temAcesso = await usuarioTemAcessoPagina(supabase, perfil?.tipo_usuario, 'ebd', 'cadastro')
  ...
  if (busca) {
    ...
    query = query.or(`nome_completo.ilike.%${termo}%,email.ilike.%${termo}%,tipo_usuario.ilike.%${termo}%`)
  }
                    ...
                    return (
                      <tr key={user.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-6 py-4 text-right">
                          <Link 
                            href={`/aplicacao/ebd/admin/cadastro/${user.id}`} 
