...
/**
 * app/aplicacao/reunioes/admin/secretaria/page.jsx
 * Hub da Secretaria — navegação + gestão de reuniões (abertas/histórico/inativar)
 * (protegido pelo layout.jsx do grupo secretaria/)
 */

import { useState, useEffect } from 'react'
import { useRouter } from 'next/navigation'
import Image from 'next/image'
import { sairSecretaria } from '@/app/aplicacao/actions/secretaria-auth'
import {
  listarReunioesAdmin, criarReuniao as criarReuniaoAction, alterarStatusReuniao,
} from '@/app/aplicacao/actions/reunioes-admin'
...
const BASE = '/aplicacao/reunioes/admin/secretaria'
...
export default function SecretariaPage() {
  ...
  async function criarReuniao() {
    ...
    const res = await criarReuniaoAction({
      titulo: form.titulo, data_reuniao: form.data_reuniao,
  ...
  async function encerrarReuniao(id) {
    ...
    setSalvando(false)
    if (res.ok) { setModal(null); mostrarToast('Reunião encerrada.', 'info'); carregar() }
    else mostrarToast(res.error || 'Erro ao encerrar.', 'erro')
  }

  async function reabrirReuniao(id) {
    const res = await alterarStatusReuniao(id, 'reabrir')
    if (res.ok) { mostrarToast('Reunião reaberta!', 'sucesso'); carregar() }
    else mostrarToast(res.error || 'Erro ao reabrir.', 'erro')
  }

  async function inativarReuniao(id) {
    const res = await alterarStatusReuniao(id, 'inativar')
    if (res.ok) { setModal(null); mostrarToast('Reunião inativada.', 'info'); carregar() }
    else mostrarToast(res.error || 'Erro ao inativar.', 'erro')
  }
    ...
    <div style={s.container}>
      ...
      <div style={s.header}>
        ...
        <div style={s.headerAcoes}>
          <button style={s.btnNova} onClick={() => setModal('nova')}>+ Nova reunião</button>
      ...
      <div style={s.body}>
        ...
        <div style={s.tituloArea}>
          <div style={s.tituloAreaNome}>Secretaria</div>
          <div style={s.tituloAreaSub}>Relatórios, cadastros e gestão</div>
        </div>

        {/* Navegação */}
        <div style={s.navBtns}>
          <button style={{ ...s.navBtn, borderColor: '#BFDBFE', background: '#EFF6FF' }} onClick={() => router.push(`${BASE}/dashboard`)}>
            <div style={{ ...s.navIcone, background: '#DBEAFE', color: '#1E40AF' }}>◎</div>
            <div>
              <div style={s.navTitulo}>Dashboard</div>
              <div style={s.navSub}>Relatórios e gráficos</div>
            </div>
            <span style={s.navSeta}>›</span>
          </button>

          <button style={{ ...s.navBtn, borderColor: '#EDE9FE', background: '#F5F3FF' }} onClick={() => router.push(`${BASE}/obreiros`)}>
          ...
          <button style={{ ...s.navBtn, borderColor: '#FDE68A', background: '#FFFBEB' }} onClick={() => router.push(`${BASE}/log`)}>
            <div style={{ ...s.navIcone, background: '#FEF3C7', color: '#92400E' }}>📋</div>
            <div>
              <div style={s.navTitulo}>Log do sistema</div>
              <div style={s.navSub}>Acessos e alterações</div>
            </div>
            <span style={s.navSeta}>›</span>
          </button>

          <button style={s.navBtn} onClick={() => router.push(`${BASE}/configuracoes`)}>
            <div style={{ ...s.navIcone, background: '#F3F4F6', color: '#374151' }}>⚙</div>
            <div>
              <div style={s.navTitulo}>Configurações</div>
              <div style={s.navSub}>Congregações, cargos, funções</div>
            </div>
            <span style={s.navSeta}>›</span>
          </button>
        </div>
        ...
        {loading ? (
          ...
          reunioesAbertas.map(r => {
            ...
            return (
              <div key={r.id} style={{ ...s.card, border: '1px solid #86EFAC', background: '#F0FDF4' }}>
                <div style={s.cardTop}>
                ...
                <div style={s.acoes}>
                  <button style={s.btnAcao} onClick={() => router.push('/aplicacao/reunioes/admin')}>Ir para check-in</button>
                ...
                <div key={r.id} style={s.card}>
                  ...
                  <div style={s.acoes}>
                    ...
                    <button style={{ ...s.btnAcao, color: '#DC2626' }}
                      onClick={() => { if (confirm('Inativar esta reunião? Ela sairá da listagem, mas os check-ins são mantidos.')) inativarReuniao(r.id) }}>
                      Inativar
                    </button>
                  </div>
                </div>
...
const s = {
  ...
  body:         { padding: '16px' },
  tituloArea:   { marginBottom: 16 },
  tituloAreaNome: { fontSize: 20, fontWeight: 700, color: '#111827' },
  tituloAreaSub:  { fontSize: 13, color: '#9CA3AF', marginTop: 2 },
