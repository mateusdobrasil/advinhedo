...
export async function criarCobranca(formData: FormData) {
  ...
  const { error } = id
    ? await supabase.from(tabela).update({ aluno_id, descricao, valor, data_vencimento }).eq('id', id)
    : await supabase.from(tabela).insert({ aluno_id, descricao, valor, data_vencimento, status: 'Pendente' })
  ...
  if (session) {
    ...
    await logAction(supabase, session.user, {
      action: id ? 'EDIÇÃO DE COBRANÇA' : 'CRIAÇÃO DE COBRANÇA',
      tableName: tabela,
      details: `${id ? 'Alterou' : 'Criou'} cobrança de R$ ${valor.toFixed(2)} (${descricao}) para o aluno ${aluno?.nome_completo || aluno_id}.`
    })
  }
