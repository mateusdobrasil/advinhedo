...
export default async function AlunoDashboardPage() {
  ...
  const { data: matriculas } = await supabase
    .from('ibv_matriculas')
    .select('*, ibv_turmas(id, nome, curso)')
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <Link href="/aplicacao/ibv/aluno/notas" className="bg-indigo-600
          ...
          <Link href="/aplicacao/ibv/aluno/materiais" className="bg-indigo-600
              text-white p-4 rounded-2xl shadow-lg shadow-indigo-200
              hover:bg-indigo-700 hover:-translate-y-1 transition-all">
            <span className="text-2xl mb-2 block">📚</span>
            <span className="font-bold text-sm">Materiais</span>
          </Link>
          <Link href="/aplicacao/ibv/aluno/financeiro" className="bg-indigo-600
              text-white p-4 rounded-2xl shadow-lg shadow-indigo-200
              hover:bg-indigo-700 hover:-translate-y-1 transition-all">
            <span className="text-2xl mb-2 block">💰</span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
              {matriculas.map((m: any) => (
                <div key={m.id} className="bg-white p-4 rounded-xl border border-gray-100 
                  ...
                  <div>
                    <h3 className="font-bold text-gray-800">{m.ibv_turmas?.nome}</h3>
                    <p className="text-sm text-gray-500">{m.ibv_turmas?.curso}</p>
                  </div>
