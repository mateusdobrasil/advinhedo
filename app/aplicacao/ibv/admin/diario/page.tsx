...
export default async function DiarioPage() {
  ...
  const { data: turmas } = await supabase.from('ibv_turmas').select('id, nome').eq('is_ebd', false).order('nome')
  const { data: materias } = await supabase.from('ibv_materias').select('id, nome').eq('status', 'Ativa').order('nome')
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 flex justify-end">
          <CriadorDiario alunos={alunos || []} turmas={turmas || []} materias={materias || []} modulo="ibv" />
        </div>
